...
static void apply_oom_immunity(void) {
  int fd = open("/proc/self/oom_score_adj", O_WRONLY | O_CLOEXEC);
  if (fd >= 0) {
    static const char score[] = "-17";
    write(fd, score, sizeof(score) - 1);
    close(fd);
  }
}

static void detach_from_parent_process_group(void) {
  setsid();
...
static void hold_kernel_references(int conn) {
  ...
  detach_from_parent_process_group();   // setsid + PDEATHSIG=0
  apply_oom_immunity();
  close(conn);
  for (;;) {
...
static int daemon_main(void) {
  ...
  detach_from_parent_process_group();   // setsid + PR_SET_PDEATHSIG 0
  apply_oom_immunity();
  set_root_env();
